#include <touchgfx/Font.hpp>

FONT_KERNING_LOCATION_FLASH_PRAGMA
KEEP extern const touchgfx::KerningNode kerning_digitalicg_40_4bpp[] FONT_KERNING_LOCATION_FLASH_ATTRIBUTE =
{
    { 0x004C, -4 },  // (First char = [0x004C, L], Second char = [0x0054, T], Kerning dist = -4)
    { 0x0054, -4 },  // (First char = [0x0054, T], Second char = [0x0061, a], Kerning dist = -4)
    { 0x0054, -3 },  // (First char = [0x0054, T], Second char = [0x0063, c], Kerning dist = -3)
    { 0x0054, -4 },  // (First char = [0x0054, T], Second char = [0x0065, e], Kerning dist = -4)
    { 0x0054, -3 },  // (First char = [0x0054, T], Second char = [0x0068, h], Kerning dist = -3)
    { 0x0054, -4 },  // (First char = [0x0054, T], Second char = [0x0069, i], Kerning dist = -4)
    { 0x0054, -4 },  // (First char = [0x0054, T], Second char = [0x006F, o], Kerning dist = -4)
    { 0x0054, -4 },  // (First char = [0x0054, T], Second char = [0x0072, r], Kerning dist = -4)
    { 0x0054, -4 },  // (First char = [0x0054, T], Second char = [0x0073, s], Kerning dist = -4)
    { 0x006C, -4 },  // (First char = [0x006C, l], Second char = [0x0074, t], Kerning dist = -4)
    { 0x0054, -4 },  // (First char = [0x0054, T], Second char = [0x0075, u], Kerning dist = -4)
    { 0x0054, -4 },  // (First char = [0x0054, T], Second char = [0x0077, w], Kerning dist = -4)
    { 0x0054, -4 },  // (First char = [0x0054, T], Second char = [0x0079, y], Kerning dist = -4)
};
